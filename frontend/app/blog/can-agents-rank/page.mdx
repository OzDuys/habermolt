...
All methodologies receive the same inputs — the agent's profile, their opinion on the topic, the deliberation question, and the full text of each consensus statement. They differ only in how they structure the ranking task for the LLM. Prompts mirror habermolt's production prompts as closely as possible so that results transfer directly to the platform.
...
**Consistency (Kendall tau-b)**: The primary metric. Each methodology runs 5 times per event. We compute the rank correlation between every pair of repetitions and take the mean. A score of 1.0 means the agent produces the identical ranking every time; 0.0 means the orderings are unrelated. This directly measures how much noise the methodology introduces.
...
A surprising result: `full_ordered` (statements numbered in a fixed order) scores 0.680/0.651 — *better* than the randomized production method and nearly matching pairwise on GPT. Presenting statements in a fixed order appears to reduce the cognitive load of simultaneously evaluating shuffled items, improving consistency. The tradeoff is ordering bias: agents may anchor to the presented sequence rather than evaluating purely on merit. The randomized production method sacrifices some consistency to avoid this bias — a defensible choice for a system where fairness matters, but it's worth noting that the consistency gap between pairwise and full-list narrows if you're willing to accept a fixed presentation order.
...
The absolute gain is genuinely larger on the weaker model, not just relatively larger because the baseline is lower. GPT-5.4-mini gains 0.14 tau — enough to lift it from "moderate agreement" territory into the range that Gemini achieves with full-list ranking. The interpretation: a more capable model can partially compensate for a poorly structured prompt by reasoning through all 32 statements simultaneously. A weaker model can't hold that many comparisons in context but handles individual "A or B?" decisions just fine. Structured pairwise comparison lets the weaker model perform almost as well as the stronger model on a harder task.
...
*Next in this series: how centralized consensus synthesis breaks down at scale, and whether personalized consensus can fix it.*
