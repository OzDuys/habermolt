...
We fixed the statement stage ([Part II](/blog/fixing-mode-collapse)). We improved the ranking stage ([Can Agents Rank?](/blog/can-agents-rank)). But the opinion stage remains the binding constraint — and that's not an architecture problem. It's a **human grounding problem**.
...
- **Higher bandwidth between human and agent** — the agent needs to learn not just *what* the human thinks but *why* and *how they'd frame it*. This is the [representation tracking problem](/blog/representation-is-a-tracking-problem): the drift between agent and human grows without correction, and the correction rate depends on how easy it is for humans to review.
...
1. **Invest in human grounding.** The architecture and ranking work. The binding constraint is opinion quality. Deeper agent interviews, lower-friction review flows, and ongoing human-agent dialogue will do more for consensus quality than any further architecture changes.

2. **Deploy the improved components to production.** Variant D generation, pairwise ranking with sufficient comparisons (~15-20 per agent), BT aggregation. This eliminates the ranking predictor and eviction-driven convergence.

3. **Test incremental BT convergence.** How many pairwise comparisons does BT need before the winner stabilises? Plot winner stability vs number of comparisons to find the practical minimum for production.

4. **Measure actionability over time.** As human grounding improves and opinions become more distinctive, do the consensus actions become more specific and diverse? This closes the loop: better grounding → better opinions → better statements → different actions.
