...
function DeliberationCard({
  ...
  return (
    ...
    >
      ...
      >
        {/* Left: question, creator, categories, badges */}
        ...
        <div className="flex shrink-0 items-center gap-4">
          <div className="flex gap-3 text-center">
            <div>
              ...
              <p className="mt-0.5 text-[10px] uppercase tracking-wide" style={{ color: "var(--muted)" }}>
                agents
              </p>
            </div>
            <div>
              <p className="text-lg font-semibold leading-none" style={{ color: "var(--foreground)" }}>
                {delib.num_statements_ranked}
              </p>
              <p className="mt-0.5 text-[10px] uppercase tracking-wide" style={{ color: "var(--muted)" }}>
                ranked
              </p>
        ...
        <div className="border-t px-5 pb-5" style={{ borderColor: "var(--border)" }}>
          ...
          {delib.winning_statement_text && (
            <div className="mt-4">
              ...
              >
                {delib.winning_statement_title && (
                  <p className="mb-1 font-medium">{delib.winning_statement_title}</p>
                )}
                ...
                >
                  ...
                  <div className="space-y-2.5">
                    <DimensionRating
                      ...
                      description="Does it fairly reflect the group's views?"
                      value={consensusRepresentativeness}
                      onChange={setConsensusRepresentativeness}
                      interactive
                    />
                    <DimensionRating
                      label="Specificity"
                      description="Is it concrete and actionable, not vague?"
                      value={consensusSpecificity}
                      onChange={setConsensusSpecificity}
                      interactive
                    />
                    <DimensionRating
                      label="Usefulness"
                      description="Would you act on this or share it?"
                      value={consensusUsefulness}
                      onChange={setConsensusUsefulness}
                      interactive
                    />
                  </div>
                  <textarea
                  ...
                  <button
                    onClick={() =>
                      onRateConsensus(
                        ...
                        {
                          representativeness: consensusRepresentativeness,
                          specificity: consensusSpecificity,
                          usefulness: consensusUsefulness,
                        },
                        consensusFeedback || undefined
                      )
                    }
                    disabled={
                      (consensusRepresentativeness === 0 || consensusSpecificity === 0 || consensusUsefulness === 0) ||
                      ratingLoading === `consensus-${delib.deliberation_id}`
                    }
            ...
            <div className="mt-4">
              ...
              {delib.opinion_submitted_at && (
                <p className="mt-1 text-xs" style={{ color: "var(--muted)" }}>
                  {formatDate(delib.opinion_submitted_at)}
                </p>
              )}
            </div>
          )}
            ...
            <div className="mt-5">
              ...
              <div className="space-y-2">
                {delib.rankings.map((item) => {
                  const rankDiff =
                    item.social_ranking !== null
                      ? item.agent_rank - item.social_ranking
                      : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        >
                          {item.statement_text.length > 200
                            ? item.statement_text.slice(0, 200) + "..."
                            : item.statement_text}
                        </p>
                        <div className="mt-1 flex gap-2">
                          {item.contributed_by_agent && (
                          ...
                          {rankDiff !== null && rankDiff !== 0 && (
                            <span
                              ...
                              style={{
                                color: rankDiff < 0
                                  ? "#16a34a"  // agent ranked higher than consensus
                                  : "#dc2626", // agent ranked lower than consensus
                              }}
                            >
                              {rankDiff < 0
                                ? `Agent ranked ${Math.abs(rankDiff)} higher`
                                : `Agent ranked ${rankDiff} lower`}
                            </span>
                          )}
                        </div>
                      </div>
          ...
          {delib.proposed_statements.length > 0 && (
            <div className="mt-5">
              <h4 className="mb-2 text-sm font-semibold" style={{ color: "var(--foreground)" }}>
                Statements proposed by your agent
              </h4>
              {delib.proposed_statements.map((stmt, i) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {stmt.social_ranking !== null && (
                      <RankBadge rank={stmt.social_ranking} label="Consensus" />
                    )}
                  </div>
            ...
            {!showRating && !delib.my_rating ? (
              ...
              <div>
                ...
                <button
                  ...
                  style={{ background: "var(--accent)" }}
                >
                  {ratingLoading === delib.deliberation_id
                    ? "Saving..."
                    : delib.my_rating
                      ? "Update rating"
                      : "Submit rating"}
...
export default function AgentActivityPage() {
        ...
        setActivity((prev) => {
          ...
          return {
            ...
            deliberations: prev.deliberations.map((d) =>
              d.deliberation_id === deliberationId
                ? { ...d, my_rating: ratingData }
                : d
            ),
          };
        });
      ...
      if (res.ok) {
        ...
        setActivity((prev) => {
          ...
          return {
            ...
            deliberations: prev.deliberations.map((d) =>
              d.deliberation_id === deliberationId
                ? { ...d, my_consensus_rating: ratingData }
                : d
            ),
          };
        });
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-12">
        <div className="animate-pulse space-y-4">
          ...
          <div className="h-4 w-96 rounded" style={{ background: "var(--surface-dim)" }} />
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <Link
        href="/profile"
        className="mb-6 inline-flex items-center gap-1 text-sm transition-opacity hover:opacity-80"
        style={{ color: "var(--accent)" }}
      >
        ← Back to profile
      </Link>
          ...
          <>
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs" style={{ color: "var(--muted)" }}>
                  ({activity.total_ratings} ratings)
                </span>
              </div>
            </div>
          </>
