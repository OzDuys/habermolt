...
Usage:
    python scripts/integration_test_continuous.py
"""
...
def register_agents(num_agents: int = 3) -> List[Dict]:
    ...
    agents = []
    agent_names = [
        ("ContAliceAgent", "Alice"),
        ("ContBobAgent", "Bob"),
        ("ContCharlieAgent", "Charlie"),
    ]
...
def create_continuous_deliberation(agent: Dict) -> str:
    ...
    question = "What policies should cities adopt to balance housing affordability with sustainable urban development?"
...
def submit_opinions(delib_id: str, agents: List[Dict]):
    ...
    opinions = [
        "Cities should implement inclusionary zoning that requires developers to include affordable units in new projects. Combined with transit-oriented development, this can create dense, walkable neighborhoods that are both sustainable and affordable.",
        "Market-based solutions work best. Reduce regulations and permit requirements to let developers build more housing faster. Government intervention like rent control actually makes affordability worse by reducing supply.",
        "We need a mixed approach: community land trusts to keep housing permanently affordable, green building standards for sustainability, and public transit investment. The key is community involvement in planning decisions.",
    ]
...
def add_statement(delib_id: str, agent: Dict) -> Dict:
    ...
    statement_text = (
        "Cities should create a balanced framework combining inclusionary zoning "
        "with streamlined permitting, funded by a land value tax that captures "
        "development gains for affordable housing and green infrastructure investment."
    )
    ...
    try:
        ...
        response = requests.post(
            ...
            headers={"X-API-Key": agent["api_key"]},
            json={"statement_text": statement_text}
        )
        response.raise_for_status()
        stmt = response.json()

        print_success(f"{agent['name']} added statement: {stmt['id']}")
        print_info(f"  \"{statement_text[:80]}...\"")
